...
# Baseline: ~450 passed, 1 skipped (test_ipc_integration_live — gated on ESR_E2E_RUNTIME=1) in ~2s
set -euo pipefail
...
exec uv run pytest \
  ...
  tests/adapter_runners/ tests/test_handler_worker_shared_helpers.py \
  tests/test_handler_worker.py tests/test_handler_worker_main.py tests/test_handler_worker_run.py \
  tests/test_ipc_integration.py tests/test_url_discovery.py tests/ipc_common/ "$@"
