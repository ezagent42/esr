...
3. **没有 PTY URL 入口** —— 加 `/pty:list` + `/pty:attach pty=<id>` 返回签名 token URL；`/cc:tui name=<agent>` 是其薄壳 shortcut
4. **`EsrWeb.PtySocket` 零认证** —— 改为 Phoenix.Token 签名验证（10 分钟 TTL）
...
- **D3.** PtySocket 仍 ALL-by-token，**不绑定 user identity**。理由：URL 设计上是可分享的（操作员粘到另一标签或发飞书给队友）；绑用户反而失去用途。token 的 expiry + 单 `pty_actor_id` scope 是安全保障。
- **D5.** `actor_ids` 字段加到 `%Esr.Entity.Agent.Instance{}` struct 上持久化，而不是放到 ActorQuery 索引里。让标识跟数据共置。
...
总规模：~580 LOC 实现 + ~400 LOC 测试 = ~980 LOC。

## 4 个待审 open question

- **Q1.** `/agent-type:list` 还是 `/plugin:agent-types`？建议前者。
- **Q2.** `/cc:tui` 落核心还是 claude_code plugin manifest？建议本 PR 落核心，等 rev-3 机制被另一 plugin 实战验证后单独迁回 claude_code 的 `slash_routes:` 块。
- **Q3.** PtySocket auth 切换会断开任何当前打开的旧 URL 标签。dev 阶段、无 production 用户，可接受。
- **Q4.** 旧的 `Esr.Commands.Scope.List`（internal_kind `session_list`）是否删除？建议删，escript 路径走新 slash 即可，没人依赖旧的 `branches[]` shape。

回 "go" 同意进入 plan 阶段；其它回复继续讨论。
