...
**生成文件头部 banner。** 每个生成文件（上面两份文档 + 重生成的
`slash-routes.default.yaml`）开头都有 banner，标明机器产出 + 指向源头：
...
yaml 用 `#` 注释形式。CI gate 在 diff 之前会剥掉这两种形式的 banner，避免 banner
本身的小改动伪装成内容变更 —— 但**没有** banner 本身就是 CI 失败
（check_command_docs 在每个重生成文件上验证 banner 存在）。
