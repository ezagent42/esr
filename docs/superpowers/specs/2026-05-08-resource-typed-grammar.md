...
**Status:** rev-4 (post subagent code-review — see § 0.1)
...
## 0.1 rev-4 changelog (2026-05-08, post subagent code-review)

Three corrections after the implementation plan was reviewed against real code:

- **D5 amended** — `/cc:tui` → `/claude_code:tui`. The plugin manifest validator (`runtime/lib/esr/plugin/manifest.ex:340-399`) enforces every slash key under a plugin's `slash_routes:` block must start with `/<plugin_name>:` and every kind name with `<plugin_name>_`. Plugin name is `claude_code`. Aliases would loosen the rev-3 D3 invariant ("operators read a slash and know which plugin owns it"). Cost: one-time muscle-memory adjustment; tools auto-complete.
- **§ 4.4 amended (URL shape + PtyProcess pubsub topic)** — emitted URL shape is `/sessions/<pty_actor_id>/attach?sid=<pty_actor_id>`. PtyProcess's pubsub broadcast topic + Entity.Registry registration migrate from `pty:<session_id>` to `pty:<actor_id>` to fix a latent M-2 multi-agent bug (N PtyProcesses in one session previously collided on the register key + PubSub topic; second registration silently overwrote the first; broadcasts mux'd). Implementation plan Phase A.4 (~20 LOC code + ~50 LOC isolation regression test); Phase E e2e 22 verifies multi-agent attach gives distinct PTY URLs.
- **§ 4.6 amended** — `defp category_order("PTY"), do: 5 # already present` quote was stale; current ordering is `PTY = 4`. Phase B.3 renumbers: Users=1, Workspace=2, Sessions=3, Agents=4, PTY=5, Plugins=6, Capabilities=7. (No spec semantic change — just truthful documentation.)

Mechanical fixes applied directly in the implementation plan (no spec amendment needed): plan B.2 uses `Esr.Session.ChatRouting.Registry.list_sessions/2` (rename of historical `attached_sessions/2`); B.4 permission for `/session:list` is `session:default/read` per spec § 4.2 row 1; E.7 e2e uses `esr_cli exec /<slash>` (the supported primitive) for the deprecated-slash hint test.
...
- **Q3.** `/cc:tui` ships in the claude_code plugin per rev-3 plugin-cmd D3. **rev-4: renamed to `/claude_code:tui`** — see § 0.1.
...
| `/claude_code:tui` | `claude_code_tui` | `pty:<actor_id>/attach` | Thin shortcut: agent-name → PTY id → `/pty:attach`. (rev-3 used `/cc:tui`; rev-4 § 0.1 renamed.) |
...
### 4.4 URL shape + PtyProcess pubsub topic migration (rev-4)

`/pty:attach pty=<actor_id>` emits an unauthenticated URL:

```
http://<host>/sessions/<actor_id>/attach
```

**rev-4 amendment:** the URL's path segment IS the PTY actor id, not the session id. PtyProcess's existing pubsub broadcast topic (`pty:<session_id>` per `pty_process.ex:292,312`) and Entity.Registry registration key (`pty:<session_id>` per `pty_process.ex:142,155`) migrate to use the actor id (`pty:<actor_id>`).

**Why the migration is in scope:** today (post-M-2) a session may host N agents and therefore N PtyProcesses. All N register under the same `pty:<session_id>` key — second registration silently overwrites the first; broadcasts mux. Single-agent works; multi-agent attach is silently broken. The rev-4 grammar surface (`/pty:list` returns N distinct PTY ids per session) makes the bug operator-visible. The fix is a 4-line swap: PtyProcess already receives `actor_id` via `build_pty_args/4` (`instance_registry.ex:326`), it just hasn't been using it.

**PtySocket** (`pty_socket.ex:41-50`) needs no code change — its `?sid=` query param now semantically carries actor_id; `state.sid` flows into `pty:<state.sid>` subscription which matches the migrated broadcast topic.

**PtySocket auth** stays out of scope (rev-2 D3). Tracked as future hardening (`pty_attach_security_hardening` in `docs/futures/todo.md`).

### 4.5 PTY-id lookup by agent name

Today's `Esr.Entity.Agent.InstanceRegistry` doesn't persist `actor_ids` on `%Instance{}`. They surface only as a side-channel return from `add_instance_and_spawn/2`. For `/claude_code:tui name=<agent>` to resolve agent → PTY id, we need a lookup.
...
| `runtime/lib/esr/plugins/claude_code/manifest.yaml` (modify) | Add `slash_routes:` block declaring `/claude_code:tui` (rev-4 § 0.1; per rev-3 mechanism — second real consumer) |
