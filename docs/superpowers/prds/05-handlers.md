...
- [ ] Each handler installs via `esr handler install ./handlers/<name>/` and appears in `esr handler list`
- [ ] Purity: zero violations per `esr-lint handlers/`
- [ ] Integration with PRDs 04 + 06: `feishu-thread-session` invoked via feishu_app's `/new-thread` produces expected actor graph
