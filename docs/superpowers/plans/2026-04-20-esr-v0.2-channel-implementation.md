...
### Task P5-2: `/new-session` parsing (keeps `/new-thread` as v0.1 compat alias)

**Reviewer C2 adjustment**: v0.1 scenario `scenarios/e2e-feishu-cc.yaml` and
`scripts/tests/test_mock_feishu.py` still POST `/new-thread foo`. Removing
the alias between P5 and P8 (when we rewrite final_gate.sh) would
break every intermediate CI run — violating the "each phase is
independently committable" contract. v0.2 KEEPS `/new-thread` as an
alias that maps to the same InvokeCommand path; P9 removes the alias
only after the scenario YAML is updated. Tests in this task cover BOTH
prefixes.
...
def _handle_new_session(
    ...
    *, prefix: str, default_workspace: str = ""
    ...
    if not workspace or not tag:
        # /new-session requires workspace; tag defaults below if blank.
        if not workspace:
            return state, []
...
```elixir
  ...
  # v0.2 §3.3 + reviewer C3: exact-match on the parsed app_id suffix,
  # not String.contains? — otherwise "cli_a9" matches "cli_a9563..."
  # and unregistered apps silently pass.
  defp check_ws_app_ids(ws) do
    # Topic format is `adapter:feishu/feishu-app:<app_id>`; extract the
    # exact app_id component.
    live_app_ids =
...
**Reviewer C4 correction**: the emit path allocates a `directive_id`
and awaits `directive_ack` asynchronously via PubSub. The naive
approach (emit → return ok=true immediately) loses the ack and reports
false success. v0.2 contract: `tool_invoke` blocks until directive_ack
(or directive_timeout) arrives, THEN pushes `tool_result` to the WS.
Implementation uses a per-peer `pending_tool_reqs: %{directive_id =>
{req_id, reply_pid}}` map + handle_info on `{:directive_ack, envelope}`
(already used by v0.1 PubSub subscription) that looks up the matching
req_id and pushes tool_result back through `reply_pid`.
...
```elixir
  # C4 — tool_invoke handler: issue Emit, then WAIT for directive_ack
  # before replying tool_result. The emit branch allocates a
  # directive_id and subscribes to "directive_ack:<id>" on PubSub; we
  # capture that id and correlate it back to the incoming req_id in
  # pending_tool_reqs. handle_info({:directive_ack, ...}) (already
  # defined in v0.1 at peer_server.ex:218) gains a new clause that
  # checks pending_tool_reqs before the normal emit bookkeeping.
  def handle_info({:tool_invoke, req_id, tool, args, reply_pid},
                  %__MODULE__{} = state) do
    case build_emit_for_tool(tool, args, state) do
      {:ok, emit} ->
        # Allocate directive_id via the same path dispatch_action uses.
        # We invoke a new helper that returns {:ok, directive_id, state'}
        # so we can record the mapping BEFORE the ack can race back.
        {:ok, directive_id, new_state} = emit_and_track(emit, state)

        pending = Map.put(new_state.pending_tool_reqs || %{}, directive_id,
                          {req_id, reply_pid})
        {:noreply, %{new_state | pending_tool_reqs: pending}}
  ...
  # Extend the existing directive_ack handler. Priority order:
  # 1. If the id is in pending_tool_reqs → it's a tool_invoke answer,
  #    push tool_result to the waiting WS reply_pid, drop normal emit
  #    bookkeeping (the emit path's pending_directives is already
  #    cleaned below).
  # 2. Fall through to the v0.1 emit behaviour for Emit(feishu, ...)
  #    that didn't come from a tool_invoke.
  def handle_info({:directive_ack, %{"id" => id} = envelope},
    ...
    # Clean both maps
    {:noreply, %{state | pending_tool_reqs: Map.delete(pending, id),
                         pending_directives: Map.delete(state.pending_directives, id)}}
  end

  # build_emit_for_tool + emit_and_track are small helpers that the
  # plan's P6-2 short-circuit also uses. Define them alongside
  # dispatch_action.

  defp build_emit_for_tool("reply", args, _state) do
    case args do
      %{"chat_id" => chat_id, "text" => text} when is_binary(chat_id) and is_binary(text) ->
        {:ok, %{"type" => "emit", "adapter" => "feishu",
                "action" => "send_message",
                "args" => %{"chat_id" => chat_id, "content" => text}}}
      _ -> {:error, "reply requires chat_id + text"}
    end
  end
  defp build_emit_for_tool("react", args, _state) do
    case args do
      %{"message_id" => mid, "emoji_type" => emoji} ->
        {:ok, %{"type" => "emit", "adapter" => "feishu", "action" => "react",
                "args" => %{"message_id" => mid, "emoji_type" => emoji}}}
      _ -> {:error, "react requires message_id + emoji_type"}
    end
  end
  defp build_emit_for_tool("send_file", args, _state) do
    case args do
      %{"chat_id" => cid, "file_path" => fp} ->
        {:ok, %{"type" => "emit", "adapter" => "feishu", "action" => "send_file",
                "args" => %{"chat_id" => cid, "file_path" => fp}}}
      _ -> {:error, "send_file requires chat_id + file_path"}
    end
  end
  ...
  # emit_and_track is the v0.1 emit path refactored to expose the
  # allocated directive_id. Refactor the existing dispatch_action
  # emit-branch to route through this helper.
  defp emit_and_track(%{"adapter" => adapter} = action, %__MODULE__{} = state) do
    ...
    envelope = %{
      "kind" => "directive", "id" => id, "ts" => DateTime.utc_now() |> DateTime.to_iso8601(),
      "type" => "directive", "source" => "esr://localhost/actor/" <> state.actor_id,
      "payload" => %{"adapter" => adapter, "action" => action["action"],
                     "args" => Map.get(action, "args", %{})}
    }
    EsrWeb.Endpoint.broadcast(topic, "envelope", envelope)
    :telemetry.execute([:esr, :emit, :dispatched], %{}, %{
      actor_id: state.actor_id, adapter: adapter, action: action["action"],
      directive_id: id
    })

    {:ok, id, %{state | pending_directives:
                         Map.put(state.pending_directives, id, %{action: action})}}
  end
  ...
  if Mix.env() == :test do
    @doc false
    def invoke_tool_for_test(state, tool, args) do
      case build_emit_for_tool(tool, args, state) do
        {:ok, _emit} -> %{"ok" => true}  # Simpler stub for unit assertions;
        # full tool_result roundtrip is tested in P6-5 integration.
...
### Task P6-5: End-to-end integration test (reviewer S7)

**Files:**
- Create: `runtime/test/esr_web/channel_integration_test.exs`

Covers the full `ChannelChannel → PeerServer → feishu_adapter_mock →
directive_ack → tool_result → ChannelChannel push` loop plus the
reverse `SessionRegistry.notify_session → ChannelChannel push_envelope`
path. No real adapter_runner subprocess — uses a fake Phoenix channel
participant subscribed to `adapter:feishu/feishu-app:test` that replies
to every broadcast with a directive_ack.

- [ ] **Step 1: Write the test**

```elixir
# runtime/test/esr_web/channel_integration_test.exs
defmodule EsrWeb.ChannelIntegrationTest do
  ...
  test "tool_invoke reply round-trips through feishu fake adapter" do
    ...
    # Start a real feishu_thread_proxy PeerServer
    actor_id = "thread:#{sid}"
    {:ok, _pid} = start_supervised({Esr.PeerServer,
      actor_id: actor_id, actor_type: "feishu_thread_proxy",
      handler_module: "feishu_thread",
      initial_state: %{"chat_id" => "oc_int"}
    })

    # Bind HubRegistry so Emit resolves.
    :ok = Esr.AdapterHub.Registry.bind(
      "adapter:feishu/feishu-app:cli_int", "feishu-app:cli_int")

    # Fake adapter: subscribe to the topic, reply to each broadcast
    # with a directive_ack PubSub message.
    parent = self()
    fake_adapter = spawn_link(fn ->
      {:ok, _, socket} =
        EsrWeb.AdapterSocket
        |> socket("fake-adapter", %{})
        |> subscribe_and_join(EsrWeb.AdapterChannel, "adapter:feishu/feishu-app:cli_int")

      send(parent, :fake_ready)

      receive do
        %Phoenix.Socket.Broadcast{event: "envelope", payload: env} ->
          # Reply with an ack
          id = env["id"]
          Phoenix.PubSub.broadcast(EsrWeb.PubSub, "directive_ack:" <> id,
            {:directive_ack, %{"id" => id,
                               "payload" => %{"ok" => true,
    ...
    assert_receive :fake_ready, 1_000

    # Join the MCP channel side
    {:ok, _, ch_socket} =
      EsrWeb.ChannelSocket
      |> socket("mcp-client", %{})
    ...
    push(ch_socket, "envelope",
      %{"kind" => "tool_invoke", "req_id" => req_id,
        "tool" => "reply",
        "args" => %{"chat_id" => "oc_int", "text" => "hello"}})

    # Expect tool_result back
    assert_receive %Phoenix.Socket.Broadcast{event: "envelope",
      payload: %{"kind" => "tool_result", "req_id" => ^req_id,
                 "ok" => true}}, 3_000
  end
  ...
  test "notify_session pushes an inbound envelope via SessionRegistry" do
    ...
    # Channel test subscribes self() to the topic automatically.
    # Call notify_session and assert the broadcast arrives.
    :ok = SessionRegistry.notify_session(sid,
            %{"kind" => "notification", "content" => "direct-notify-" <> sid})

    assert_receive %Phoenix.Socket.Broadcast{event: "envelope",
      payload: %{"kind" => "notification", "content" => content}}, 2_000
...
```bash
cd runtime && mix test test/esr_web/channel_integration_test.exs -v
```
Expected: PASS (2 tests).
...
```bash
git add runtime/test/esr_web/channel_integration_test.exs
git commit -m "test(v0.2): end-to-end integration — tool_invoke + notify_session"
```
...
**⚠️ HUMAN-SYNC PHASE**: Tasks in P8 involve SHA-pinned files
(`scripts/final_gate.sh`, `scripts/final_gate.sh.sha256`,
`scripts/loopguard-bundle.sha256`). **Reviewer C1 correction**:
`scripts/loopguard.sh` itself does NOT contain inline hash literals —
LG-4 runs `sha256sum -c scripts/final_gate.sh.sha256` and LG-11 checks
`loopguard-bundle.sha256`. Do NOT edit `loopguard.sh` for P8/P9-3; only
update the two `.sha256` files. The ralph-loop CANNOT modify SHA-pinned
files autonomously (would trip LG-4/LG-11). Per-task workflow:
...
# Recompute the loopguard bundle hash (LG-11) — scripts/loopguard.sh
# reads this file at runtime; no inline literal to edit.
bash scripts/compute_loopguard_bundle.sh > scripts/loopguard-bundle.sha256
```
...
```bash
git add scripts/final_gate.sh scripts/final_gate.sh.sha256 scripts/loopguard-bundle.sha256
...
# LG-6 allowlist is enforced by scripts/loopguard_scenarios_allowlist.py;
# the SHA pin lives in scripts/loopguard-scenarios-allowlist.sha256 (no
# edit to loopguard.sh itself per reviewer C1).
