...
- `runtime/test/esr/application_legacy_layout_test.exs` — refuse-to-boot tests
...
- `runtime/lib/esr/plugin/config.ex` — rewrite 3-layer reader/writer
- `runtime/lib/esr/plugin/plugins_yaml.ex` — strip `:config` handling, keep enabled-list ops only
- `runtime/lib/esr/application.ex` — call `Esr.Adapters.list/0` (replaces lines 405-436); add `check_legacy_layout!/1` to `start/2`; **delete** lines 438-486 (`ensure_app_secret` feishu fallback)
- `runtime/lib/esr/commands/register_adapter.ex` — replace `append_instance_to_yaml/4` body with `Esr.Adapters.add/3` call (DSL block stays)
...
- [ ] Add `runtime/lib/esr/adapters.ex` per spec § 4.3 (all 9 functions).
- [ ] Add `Esr.Yaml.Writer.write/2` calls — verify the existing helper signature accepts the per-instance `<inst>/adapters/<name>/config.yaml` paths (no need to change Writer).
...
- [ ] `runtime/lib/esr/commands/register_adapter.ex:95-113` (`append_instance_to_yaml/4`) — replace with `Esr.Adapters.add(name, "feishu", %{"app_id" => app_id, "app_secret" => secret})`. DSL block (`command :register_adapter do …`) stays untouched.
...
- [ ] `runtime/lib/esr/plugin/config.ex` — rewrite the 3-layer reader to walk per-directory paths via new `Esr.Paths.plugin_{global,user,workspace}_dir/N` helpers. Apply merge order workspace > user > global (last-wins).
- [ ] `runtime/lib/esr/plugin/plugins_yaml.ex` — strip all `:config` handling. Reading a yaml that contains `:config` raises (will be caught by Stage 4's boot gate).
...
- [ ] `runtime/test/esr/application_restore_adapters_test.exs` — rewrite per spec § 6.3: assert `Logger.error` matches `~r/missing app_secret in adapters\/.*\/config\.yaml/`; assert `spawn_fn` not called for that row; assert other valid rows still spawn.
...
- [ ] Create `Esr.Commands.Adapter.{Disable,Enable,Remove,List}` modules. Each:
  ...
  - `command :adapter_<verb> do … end` block declaring `slash`, `category "Adapters"`, `description`, `permission "adapter.<verb>"`, `arg :name, required: true` (List has no args), `error :not_found, "adapter %{name} not found"`, `error :already_disabled, "..."`, etc. as needed.
...
## Stage 4 — Boot-time legacy detection

- [ ] Add `defp check_legacy_layout!/1` to `runtime/lib/esr/application.ex`'s `start/2`, called BEFORE `Supervisor.start_link`. Logic per spec § 5: detect stale `adapters.yaml` OR `plugins.yaml` containing `:config` key → `Logger.error(...) + System.halt(1)`.
- [ ] Add `runtime/test/esr/application_legacy_layout_test.exs` — capture-log style; place fixture yaml in tmp dir; assert halt-or-equivalent and error message.
- [ ] Smoke locally: `rm -rf ~/.esrd-test/test-v2-stale && mkdir -p ~/.esrd-test/test-v2-stale/default && touch ~/.esrd-test/test-v2-stale/default/adapters.yaml && ESRD_HOME=~/.esrd-test/test-v2-stale ESR_INSTANCE=default mix run --no-halt` should exit 1 with the cleanup hint.
- [ ] Verify: `mix test test/esr/application_legacy_layout_test.exs` passes.

**Done when**: boot refuses on legacy, prints actionable cleanup, doesn't trip on a clean v2 layout.

## Stage 5 — E2E + operator docs

- [ ] Add `tests/e2e/scenarios/<NN>_yaml_layout_v2.sh` per spec § 6.2. Update both the `README.md` "E2E test scenarios" table AND `docs/architecture.md` "E2E coverage map" table per repo CLAUDE.md rule.
...
- **`check_legacy_layout!` in Stage 4 must not run during test mode** — tests use ephemeral homes that may not have the v2 dirs created yet. Skip the gate when `Mix.env() == :test` OR when `ESRD_HOME` points inside `_build/`.
...
- Do NOT add a migration script. Operators delete legacy state per the boot-time cleanup hint.
