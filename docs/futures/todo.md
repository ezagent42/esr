...
| **Migrate `.claude/worktrees/dev` → `.worktrees/dev`** | done | PR-24-cleanup-C 2026-05-02. `git worktree move` + plist `WorkingDirectory` + `ESR_REPO_DIR` updated + `launchctl bootout`/`bootstrap`. `scripts/launchd/install.sh` + `uninstall.sh` `DEV_WORKTREE` paths fixed; operator docs (dev-flow.md, dev-prod-isolation.md) updated. esrd-dev cwd now `/Users/h2oslabs/Workspace/esr/.worktrees/dev/runtime`. |
...
| **esbuild bundle isn't auto-rebuilt on deploy** | done | PR-24-cleanup-A 2026-05-02 (PR #157). `config/dev.exs` watchers gained `esbuild: {Esbuild, :install_and_run, [:default, ~w(--watch)]}` so launchd-restart triggers an immediate rebuild + watcher continues incrementally. `mix.exs` aliases gained `assets.deploy` (`esbuild default --minify` + `phx.digest`) for prod release builds. |
