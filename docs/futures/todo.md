...
| **Multi-agent: metadata-vs-runtime gap [待检查]** | New 2026-05-07 (surfaced during legacy routing-diffusion audit) | `Esr.Entity.Agent.InstanceRegistry` (built tonight) lets `/session:add-agent <name>` insert instance metadata into ETS, but `Esr.Session.AgentSpawner.create_session/1` still spawns one fixed pipeline (FAA→FCP→CC→PTY) from `agents.yaml` `pipeline.inbound[]` at session-creation time. Result: **`/session:add-agent` makes the agent visible in InstanceRegistry but no runtime CC/PTY peer exists for it** — `MentionParser` resolves `@<name>` to a name with no pid behind it. Root cause: PR-3's `state.neighbors` keyword list keyed by role atom forced "1 peer per role per session", and `agent_def.pipeline.inbound[].name` doubles as the role atom in `state.neighbors`. Brainstorm input material: `docs/futures/multi-instance-routing-cleanup.md`. **待检查**：session-first model migration (above row) may rewrite `agent_spawner` and incidentally close part of this gap; revisit after that lands. |
