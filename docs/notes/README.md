...
| PTY attach diagnostic (2026-05-02) | [pty-attach-diagnostic.md](pty-attach-diagnostic.md) | When an esrd-spawned claude doesn't reply to a Feishu inbound, walk the chain end-to-end. Per-stage `grep` checks, `websocat` recipe to inspect the claude TUI raw stream (post PR-24 binary WS), and the `scripts/cc-bootstrap.sh` one-shot for the `--dangerously-load-development-channels` confirmation dialog. Includes the architecture cheat-sheet (Feishu→FAA→FCP→cc_process→PubSub→cc_mcp→claude→reply tool→FCP→Feishu). |
