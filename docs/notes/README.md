...
| erlexec worker lifecycle (2026-04-30) | [erlexec-worker-lifecycle.md](erlexec-worker-lifecycle.md) | PR-21β: WorkerSupervisor migrated from `spawn_worker.sh` + pidfile + `cleanup_orphans` to erlexec-managed DynamicSupervisor. BEAM owns subprocess lifecycle; zero orphans possible. Includes the 8x-orphan incident retro that motivated the rewrite. |
| PTY attach diagnostic (2026-05-02) | [pty-attach-diagnostic.md](pty-attach-diagnostic.md) | When an esrd-spawned claude doesn't reply to a Feishu inbound, walk the chain end-to-end. Per-stage `grep` checks, `websocat` recipe to inspect the claude TUI raw stream (post PR-24 binary WS), and the operator path for the `--dangerously-load-development-channels` confirmation dialog (open `/sessions/<sid>/attach` and answer `1` + Enter; e2e tests use `tests/e2e/_helpers/dev_channels_unblock.sh`). Includes the architecture cheat-sheet (Feishu→FAA→FCP→cc_process→PubSub→cc_mcp→claude→reply tool→FCP→Feishu). |
| Channel-server port debt audit (2026-05-05) | [2026-05-05-channel-port-debt-audit.md](2026-05-05-channel-port-debt-audit.md) ([中文](2026-05-05-channel-port-debt-audit.zh_cn.md)) | Pre-Phase-2/3 audit of debt sediment from the channel-server port + Python residue. Headline: **no BLOCKING debt**; 2 immediate-delete files (`daemon.py` + `dev_channels_unblock.sh` = ~302 LOC); everything else cleaned ALONG-THE-WAY by Phase 2/3 or deferred to Phase 4. Includes 3 cleanup options (A conservative / B moderate / C rejected). |
| Phase 2 complete — slash/CLI unification (2026-05-05) | [2026-05-05-phase-2-complete.md](2026-05-05-phase-2-complete.md) | Final note for Phase 2: 9 PRs (#193–#201). Single dispatch path via `Esr.Entity.SlashHandler`. ReplyTarget DI at every reply boundary. `esr` Elixir-native escript replaces Python core CLI. Net +200 LOC; voice removal balanced new module surface. PR-2.7 verification (runtime.exs reads `plugins.yaml`) — already live since Track 0 Task 0.5. |
| Phase 3 + Phase 4 status (2026-05-05) | [2026-05-05-phase-3-4-status.md](2026-05-05-phase-3-4-status.md) | Phase 3 (5/7 shipped, PR-3.4 + PR-3.5 deferred) + Phase 4 (3/7, PR-4.1 partial, 3 deferred). **Partial migration, not done**: feishu module *files* moved under `plugins/feishu/` but `bootstrap_feishu_app_adapters/0` still owned by core (PR-3.4 pending); cc_mcp still stdio child of claude (PR-3.5 pending); Python CLI fully intact — escript covers ~6/31 click commands (PR-4.6/4.7 untouched). Cumulative session: 22 PRs (#189–#209). |
| CLI dual-rail e2e gate (2026-05-05) | [2026-05-05-cli-dual-rail.md](2026-05-05-cli-dual-rail.md) ([中文](2026-05-05-cli-dual-rail.zh_cn.md)) | Phase A of Phase 3/4 finish: `esr_cli()` helper in common.sh, `RUN_VIA={python,escript}` toggle, 27 inline `uv run` calls switched. Escript rail expected red on `actors list`, output-format mismatch, etc. — that IS the migration progress signal. Each Phase B PR turns one assertion green; Phase C deletes Python CLI when escript rail fully green. |
