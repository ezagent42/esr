defmodule Esr.EntityServerPersistTest do
  @moduledoc """
  PRD 01 F18 + spec §7.4 — Entity.Server persists `new_state` to
  ...
  defp start_peer(actor_id, handler, opts \\ []) do
    ...
    GenServer.whereis({:via, Registry, {Esr.Entity.Registry, actor_id}})
  end
  ...
  test "init/1 rehydrates from Persistence.Ets when the actor_id has a prior state" do
    ...
    assert Entity.Server.get_state(actor_id) == %{"counter" => 17, "name" => "kept"}
  end
