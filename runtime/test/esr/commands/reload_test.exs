defmodule Esr.Commands.ReloadTest do
  @moduledoc """
  DI-12 Task 26 — `Esr.Commands.Reload` scans
  ...
  describe "dispatcher wiring" do
    test "`reload` kind is registered in the Dispatcher's @command_modules map" do
      ...
      assert Code.ensure_loaded?(Esr.Commands.Reload)
      assert function_exported?(Esr.Commands.Reload, :execute, 1)
      assert function_exported?(Esr.Commands.Reload, :execute, 2)
    end
  end
end
