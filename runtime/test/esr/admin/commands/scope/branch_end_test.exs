defmodule Esr.Admin.Commands.Scope.BranchEndTest do
  @moduledoc """
  DI-10 Task 21 — `Esr.Admin.Commands.Scope.BranchEnd` (formerly
  ...
  `principals[*].targets[<branch>]` across all principals in
  ...
  `Esr.Admin.Commands.Scope.New` — tests pass a 1-arity stub that
  receives `{argv}` and returns `{stdout, exit_status}`.
  """

  use ExUnit.Case, async: false

  alias Esr.Admin.Commands.Scope.BranchEnd, as: SessionBranchEnd
  ...
  describe "execute/2 happy path" do
    ...
    test "unsets active when ended branch was submitter's active (no remaining targets)", %{
      ...
      assert {:ok, _} = SessionBranchEnd.execute(cmd, spawn_fn: stub)
