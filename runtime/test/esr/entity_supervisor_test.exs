defmodule Esr.EntitySupervisorTest do
  @moduledoc """
  PRD 01 F04 — DynamicSupervisor that owns Entity.Server children.
  ...
  setup do
    # Entity.Supervisor + Entity.Registry are started by the Application for the
    # whole test run. Clean up any peers from prior tests.
    for {actor_id, _pid} <- Esr.Entity.Registry.list_all() do
      _ = Esr.Entity.Supervisor.stop_peer(actor_id)
    end

    :ok
  end

  # Poll Entity.Registry until the key is gone or timeout. Registry cleanup
  ...
  describe "start_peer/1" do
    test "spawns a Entity.Server under the supervisor" do
      {:ok, pid} =
        Esr.Entity.Supervisor.start_peer(
          actor_id: "cc:sess-A",
      ...
      children = DynamicSupervisor.which_children(Esr.Entity.Supervisor)
  ...
  describe "stop_peer/1" do
    test "terminates the peer and unregisters it" do
      ...
      assert :ok = Esr.Entity.Supervisor.stop_peer("cc:sess-B")
      ...
      assert wait_until_unregistered("cc:sess-B", 500),
             "Entity.Registry did not unregister cc:sess-B within 500ms"
    end

    test "returns {:error, :not_found} for unknown actor_id" do
      assert {:error, :not_found} = Esr.Entity.Supervisor.stop_peer("cc:sess-does-not-exist")
    end
  end
  ...
  describe "one_for_one isolation" do
    test "sibling peer survives a co-spawned peer crash" do
      ...
      assert {:ok, ^b_pid} = Esr.Entity.Registry.lookup("cc:sibling-B")
    end
  end
end
