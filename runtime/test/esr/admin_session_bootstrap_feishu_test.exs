defmodule Esr.AdminSessionBootstrapFeishuTest do
  @moduledoc """
  PR-9 T10: `Esr.AdminSession.bootstrap_feishu_app_adapters/1` reads
  ...
  AdminSessionProcess so `EsrWeb.AdapterChannel.forward_to_new_chain/2`
  ...
  setup do
    ...
    assert is_pid(Process.whereis(Esr.AdminSession.ChildrenSupervisor))
    ...
    on_exit(fn ->
      ...
      for name <- [
        ...
        case Esr.AdminSessionProcess.admin_peer(name) do
          {:ok, pid} ->
            DynamicSupervisor.terminate_child(
              Esr.AdminSession.ChildrenSupervisor,
              pid
            )
  ...
  test "registers one FeishuAppAdapter per feishu-type instance in adapters.yaml",
    ...
    assert :ok = Esr.AdminSession.bootstrap_feishu_app_adapters(tmp)

    {:ok, main_pid} = Esr.AdminSessionProcess.admin_peer(:feishu_app_adapter_main_bot)
    {:ok, secondary_pid} = Esr.AdminSessionProcess.admin_peer(:feishu_app_adapter_secondary)
  ...
  test "is idempotent — re-running with the same adapters.yaml is :ok",
    ...
    assert :ok = Esr.AdminSession.bootstrap_feishu_app_adapters(tmp)
    {:ok, pid1} = Esr.AdminSessionProcess.admin_peer(:feishu_app_adapter_only)

    assert :ok = Esr.AdminSession.bootstrap_feishu_app_adapters(tmp)
    {:ok, pid2} = Esr.AdminSessionProcess.admin_peer(:feishu_app_adapter_only)
