defmodule Esr.PeerServerEventHandlingTest do
  @moduledoc """
  PRD 01 F06 — PeerServer.handle_info({:inbound_event, envelope}, state)
  ...
  test "inbound event triggers HandlerRouter.call and persists new_state",
    ...
    send(GenServer.whereis({:via, Registry, {Esr.PeerRegistry, actor_id}}),
         {:inbound_event, envelope})

    # Poll for state update — send is async; the inner HandlerRouter.call is sync.
    Process.sleep(100)
    assert PeerServer.get_state(actor_id) == %{"counter" => 1}
  ...
  test "duplicate event by idempotency_key is deduped", %{actor_id: actor_id} do
    ...
    peer_pid = GenServer.whereis({:via, Registry, {Esr.PeerRegistry, actor_id}})
  ...
  test "handler timeout leaves state unchanged", %{actor_id: actor_id} do
    ...
    # Allow the 200ms test-scoped timeout to elapse (see PeerServer config)
    Process.sleep(300)

    # State is unchanged from initial
    assert PeerServer.get_state(actor_id) == %{"counter" => 0}
  end
end
