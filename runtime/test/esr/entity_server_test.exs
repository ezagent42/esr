defmodule Esr.EntityServerTest do
  @moduledoc """
  PRD 01 F05 — Entity.Server skeleton. Verifies initial state is stored
  and retrievable; registers itself in Entity.Registry; emits the
  ...
  describe "start_link/1" do
    test "starts a Entity.Server and registers it in Entity.Registry" do
      {:ok, pid} =
      ...
      assert {:ok, ^pid} = Esr.Entity.Registry.lookup("test:1")
    end
  ...
  describe "get_state/1" do
    test "returns the initial_state after start" do
      ...
      assert Esr.Entity.Server.get_state("test:state") == %{counter: 7}
    end
  end
  ...
  describe "build_emit_for_tool/3 reads channel_adapter from state (D2)" do
    test "reply emit uses state.channel_adapter when set" do
      ...
      {:ok, emit} =
        Esr.Entity.Server.build_emit_for_tool_for_test(
          "reply",
