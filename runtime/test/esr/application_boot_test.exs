defmodule Esr.ApplicationBootTest do
  ...
  test "Scope.Admin, Scope.Supervisor, Scope.Router are all booted" do
    ...
    assert is_pid(Process.whereis(Esr.Session.Admin))
    ...
    assert is_pid(Process.whereis(Esr.Session.Router))
  end

  test "child order: Esr.Session.Registry < Scope.Admin < Scope.Supervisor" do
    ...
    #   Esr.Session.Registry (1st) < Esr.Session.Admin (2nd) < Esr.Session.Supervisor (3rd)
    ...
    admin_idx = Enum.find_index(ids, &(&1 == Esr.Session.Admin))
    sessions_idx = Enum.find_index(ids, &(&1 == Esr.Session.Supervisor))
