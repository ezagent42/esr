defmodule Esr.Entity.StatefulBehaviourTest do
  @moduledoc """
  P5-6 — Esr.Entity.Stateful must NOT declare @callback init/1.
  ...
  test "Esr.Entity.Stateful does not declare init/1 as a callback" do
    callbacks = Esr.Entity.Stateful.behaviour_info(:callbacks)
    refute {:init, 1} in callbacks,
      "Esr.Entity.Stateful should leave init/1 to GenServer"
  end

  test "Esr.Entity.Stateful still declares handle_upstream/2 and handle_downstream/2" do
    callbacks = Esr.Entity.Stateful.behaviour_info(:callbacks)
  ...
  describe "macro defaults" do
    ...
    test "dispatch_upstream translates :drop to {:noreply, state}" do
      assert Esr.Entity.Stateful.dispatch_upstream(:drop_me, %{a: 1}, UpPeer) ==
               {:noreply, %{a: 1}}
    end

    test "dispatch_upstream translates :forward to {:noreply, state}" do
      assert Esr.Entity.Stateful.dispatch_upstream(:forward_me, %{a: 1}, UpPeer) ==
               {:noreply, %{a: 1}}
    end

    test "dispatch_upstream translates :reply to {:noreply, state}" do
      assert Esr.Entity.Stateful.dispatch_upstream(:reply_me, %{a: 1}, UpPeer) ==
               {:noreply, %{a: 1}}
    end

    test "dispatch_upstream on a peer using default handle_upstream returns {:noreply, state}" do
      assert Esr.Entity.Stateful.dispatch_upstream(:anything, %{a: 1}, MinimalPeer) ==
               {:noreply, %{a: 1}}
    end
    ...
    test "dispatch_downstream translates :drop and :forward to {:noreply, state}" do
      assert Esr.Entity.Stateful.dispatch_downstream(:drop_me, %{a: 1}, DownPeer) ==
               {:noreply, %{a: 1}}

      assert Esr.Entity.Stateful.dispatch_downstream(:forward_me, %{a: 1}, DownPeer) ==
               {:noreply, %{a: 1}}
    end
  end
end
