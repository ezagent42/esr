defmodule Esr.SessionAdminTest do
  use ExUnit.Case, async: false

  setup do
    # Drift from expansion doc: P2-9 added `Esr.Session.Admin` to
    # `Esr.Application`'s supervision tree, registering
    # `Esr.Session.Admin.Process` and `Esr.Session.Admin.ChildrenSupervisor`
  ...
  test "Scope.Admin.children_supervisor/1 returns the DynamicSupervisor for admin peers" do
    assert is_atom(Esr.Session.Admin.children_supervisor_name(:test_admin_session))
  ...
  test "Scope.Admin starts without Esr.Session.Router running (Risk F)", %{admin_process: name} do
    ...
    assert Code.ensure_loaded?(Esr.Session.Router),
