defmodule Esr.DeadLetterTest do
  ...
  alias Esr.DeadLetter

  setup do
    pid = start_supervised!({DeadLetter, name: :dl_test, max_entries: 3})
    %{dl: pid}
  end
  ...
  describe "enqueue/2 + list/1" do
    test "stores an entry and returns it from list" do
      ...
      entries = DeadLetter.list(:dl_test)
    ...
    test "list/1 returns empty when nothing enqueued" do
      assert DeadLetter.list(:dl_test) == []
    end
  end
  ...
  describe "FIFO eviction when at capacity" do
    test "oldest entry dropped when max_entries exceeded" do
      for i <- 1..4 do
        :ok = DeadLetter.enqueue(:dl_test, %{reason: :test, target: "t#{i}", msg: i})
      end
  ...
  describe "clear/1" do
    test "removes all entries" do
      :ok = DeadLetter.enqueue(:dl_test, %{reason: :test, target: "x", msg: nil})
      assert length(DeadLetter.list(:dl_test)) == 1

      :ok = DeadLetter.clear(:dl_test)
      assert DeadLetter.list(:dl_test) == []
    end
  end
  ...
  describe "telemetry" do
    test "enqueue fires [:esr, :deadletter, :event]" do
      ...
      :ok = DeadLetter.enqueue(:dl_test, %{reason: :test, target: "y", msg: nil})
