defmodule Esr.EntityServerLaneBDenyDispatchTest do
  @moduledoc """
  ...
  process in `Esr.Entity.Registry` under the expected key, then assert
  ...
  defp register_fake_faa(instance_id) do
    # Entity.Registry.register only registers the calling pid (see its
    # moduledoc). To register the test process, we just call it from
    # here — `self()` is the test pid.
    :ok =
      case Esr.Entity.Registry.register("feishu_app_adapter_#{instance_id}", self()) do
        {:ok, _} -> :ok
    ...
    on_exit(fn -> Registry.unregister(Esr.Entity.Registry, "feishu_app_adapter_#{instance_id}") end)
    :ok
  end
  ...
  test "deny path with no FAA registered logs a warning and doesn't crash" do
    ...
    # source URI is unique and won't resolve in Entity.Registry.
