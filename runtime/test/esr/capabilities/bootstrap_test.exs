defmodule Esr.Capabilities.BootstrapTest do
  @moduledoc """
  Capabilities spec §9.1 — first-run seed-file bootstrap.

  Covers the `Esr.Capabilities.Supervisor.init/1` branch that writes
  ...
  alias Esr.Capabilities.Supervisor, as: CapSupervisor
  ...
  test "seed file parses cleanly through FileLoader", %{path: path} do
    ...
    assert :ok = Esr.Capabilities.FileLoader.load(path)
    assert Esr.Capabilities.Grants.has?("ou_integration", "workspace:proj-z/msg.send")
  end
end
