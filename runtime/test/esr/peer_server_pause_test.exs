defmodule Esr.PeerServerPauseTest do
  @moduledoc """
  PRD 01 F20 — Pause / resume. Paused PeerServer queues inbound_event
  ...
  test "pause/1 emits [:esr, :actor, :paused] telemetry" do
    ...
    assert :ok = PeerServer.pause("pause:1")
  ...
  test "resume/1 emits [:esr, :actor, :resumed] telemetry" do
    ...
    :ok = PeerServer.pause("pause:2")
    assert :ok = PeerServer.resume("pause:2")
  ...
  test "inbound_event queues while paused and drains in FIFO order on resume" do
    ...
    assert PeerServer.pending_count("pause:3") == 3

    :ok = PeerServer.resume("pause:3")
    ...
    # Queue is now empty.
    assert PeerServer.pending_count("pause:3") == 0
  end
end
