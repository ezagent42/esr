defmodule Esr.PeerServerToolInvokeTest do
  ...
  test "invoke_tool_for_test/3 returns error for missing reply args" do
    ...
    result = PeerServer.invoke_tool_for_test(peer_state, "reply", %{"chat_id" => "oc_x"})
  ...
  test "invoke_tool_for_test/3 rejects unknown tool" do
    ...
    result = PeerServer.invoke_tool_for_test(peer_state, "bogus", %{})
    assert result["ok"] == false
  ...
  test "_echo tool requires chat_id in thread state" do
    ...
    result = PeerServer.invoke_tool_for_test(peer_state, "_echo", %{"nonce" => "abc"})
    assert result["ok"] == false
  end
  ...
  test "_echo tool synthesises reply from nonce when chat_id present" do
    ...
    result = PeerServer.invoke_tool_for_test(peer_state, "_echo", %{"nonce" => "nonce-1"})
    assert result["ok"] == true
  end
end
