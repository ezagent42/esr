defmodule Esr.PeerRegistry do
  ...
  atom name `Esr.PeerRegistry`. Spec §3.2: PeerServers use `{:via,
  Registry, {Esr.PeerRegistry, actor_id}}` to register.

  The module name deliberately shadows the registered process name —
  callers write `Esr.PeerRegistry.lookup("cc:sess-A")` without having
  ...
  def register(actor_id, pid) when is_binary(actor_id) and is_pid(pid) do
    ...
    # Esr.PeerServer.init/1 calling `register(actor_id, self())`, so the
