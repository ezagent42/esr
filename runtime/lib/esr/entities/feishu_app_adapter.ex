defmodule Esr.Entities.FeishuAppAdapter do
  @moduledoc """
  ...
  **Today's architecture note**: the actual Feishu WebSocket is
  ...
  # `Esr.Entities.CapGuard`. CapGuard sends `{:outbound, %{"kind" => "reply",
  ...
  def spawn_args(params) do
    instance_id = Esr.Entity.get_param(params, :instance_id) || "default"
    app_id = Esr.Entity.get_param(params, :app_id) || instance_id
  ...
  def init(%{instance_id: instance_id} = args) do
    ...
    # via Esr.Entity.Registry under "feishu_app_adapter_<instance_id>"
    ...
    # routing uses the binary-keyed Entity.Registry. Ignore re-register
    # races so a hot-reload that re-runs init/1 doesn't crash the
    # GenServer — the existing entry stays valid.
    _ = Esr.Entity.Registry.register("feishu_app_adapter_#{instance_id}", self())
    ...
    {:ok,
     %{
       ...
       # DMs lives in `Esr.Entities.UnboundChatGuard` / `UnboundUserGuard`.
       # PR-21x: deny-DM rate-limit moved to `Esr.Entities.CapGuard`.
      ...
      _ ->
        ...
        cond do
          ...
          true ->
            ...
            # PR-21w: extracted into Esr.Entities.UnboundUserGuard.
            ...
            case Esr.Entities.UnboundUserGuard.check(user_id, chat_id, app_id) do
              {:emit, text} ->
  ...
  defp dispatch_slash(envelope, text, chat_id, message_id, state) do
    ...
    ref = make_ref()
    Esr.Entities.SlashHandler.dispatch(envelope_with_text, self(), ref)
  ...
  defp do_handle_upstream_inbound(envelope, args, chat_id, thread_id, state) do
    ...
    case Esr.SessionRegistry.lookup_by_chat(chat_id, app_id) do
      ...
      :not_found ->
        ...
        guard_result = Esr.Entities.UnboundChatGuard.check(chat_id, app_id, state.instance_id)
  ...
  # callbacks via the shared Esr.Entity.Stateful.dispatch_{upstream,downstream}/3
  # helpers (PR-6 B1).
  @impl GenServer
  ...
  def handle_info({:outbound, _envelope} = msg, state),
    do: Esr.Entity.Stateful.dispatch_downstream(msg, state, __MODULE__)
  ...
  # `Esr.Entities.CapGuard` now sends `{:outbound, ...}` directly to this
