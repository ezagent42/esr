defmodule Esr.Entities.UnboundUserGuard do
  @moduledoc """
  ...
  otherwise `Esr.Entities.UnboundChatGuard` takes precedence so
  operators don't get pelted with two DMs.

  Extracted from `Esr.Entities.FeishuAppAdapter` by PR-21w. Symmetric
  to `Esr.Entities.UnboundChatGuard` — same migration rationale, same
  shape.
  ...
  def check(user_id, chat_id, app_id)
    ...
    with {:ok, _ws} <- Esr.Resource.Workspace.Registry.workspace_for_chat(chat_id, app_id),
         registry_pid when is_pid(registry_pid) <- Process.whereis(Esr.Entity.User.Registry),
         :not_found <- Esr.Entity.User.Registry.lookup_by_feishu_id(user_id) do
