defmodule Esr.Entities.VoiceTTSProxy do
  @moduledoc """
  Per-Session `Peer.Proxy` — session-local door to the VoiceTTS pool.

  Mirror of `Esr.Entities.VoiceASRProxy` in the outbound direction:
  `forward/2` acquires a TTS worker, calls
  `Esr.Entities.VoiceTTS.synthesize/2`, releases. Message tag `:voice_tts`,
  ...
  def forward({:voice_tts, text}, %{pool_name: pool_name} = ctx) when is_atom(pool_name) do
    ...
    case Esr.Entity.Pool.acquire(pool_name, timeout: timeout) do
      {:ok, worker} ->
        try do
          case Esr.Entities.VoiceTTS.synthesize(worker, text, timeout) do
            {:ok, _} = ok -> ok
