defmodule Esr.Entity.FeishuAppAdapter do
  @moduledoc """
  ...
  **Today's architecture note**: the actual Feishu WebSocket is
  ...
  # `Esr.Entity.CapGuard`. CapGuard sends `{:outbound, %{"kind" => "reply",
  ...
  def init(%{instance_id: instance_id} = args) do
    ...
    {:ok,
     %{
       ...
       # DMs lives in `Esr.Entity.UnboundChatGuard` / `UnboundUserGuard`.
       # PR-21x: deny-DM rate-limit moved to `Esr.Entity.CapGuard`.
      ...
      _ ->
        ...
        cond do
          ...
          true ->
            ...
            # PR-21w: extracted into Esr.Entity.UnboundUserGuard.
            ...
            case Esr.Entity.UnboundUserGuard.check(user_id, chat_id, app_id) do
              {:emit, text} ->
  ...
  defp dispatch_slash(envelope, text, chat_id, message_id, state) do
    ...
    ref = make_ref()
    Esr.Entity.SlashHandler.dispatch(envelope_with_text, self(), ref)
  ...
  defp do_handle_upstream_inbound(envelope, args, chat_id, thread_id, state) do
    ...
    case Esr.SessionRegistry.lookup_by_chat(chat_id, app_id) do
      ...
      :not_found ->
        ...
        guard_result = Esr.Entity.UnboundChatGuard.check(chat_id, app_id, state.instance_id)
  ...
  # `Esr.Entity.CapGuard` now sends `{:outbound, ...}` directly to this
