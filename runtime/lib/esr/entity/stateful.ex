defmodule Esr.Entity.Stateful do
  @moduledoc """
  ...
  warning on every module that did both `use Esr.Entity.Stateful`
  ...
  `use Esr.Entity.Stateful` now injects three overridable defaults so
  ...
  Plus `Esr.Entity.Stateful.dispatch_upstream/3` (and the downstream
  ...
  defmacro __using__(_opts) do
    quote do
      use Esr.Entity, kind: :stateful
      @behaviour Esr.Entity.Stateful

      # Default dual-shape start_link: accept both `map()` (unit-test
      # convention) and `keyword()` (Esr.Entity.Pool's `worker_mod.start_link([])`
