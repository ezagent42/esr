defmodule Esr.Entity.VoiceASRProxy do
  @moduledoc """
  ...
  `Esr.Entity.VoiceASR.transcribe/2`, then releases the worker back to
  ...
  def forward({:voice_asr, audio_b64}, %{pool_name: pool_name} = ctx) when is_atom(pool_name) do
    ...
    case Esr.Entity.Pool.acquire(pool_name, timeout: timeout) do
      {:ok, worker} ->
        try do
          case Esr.Entity.VoiceASR.transcribe(worker, audio_b64, timeout) do
            {:ok, _} = ok -> ok
