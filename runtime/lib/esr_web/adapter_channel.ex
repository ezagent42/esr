defmodule EsrWeb.AdapterChannel do
  ...
  Routing (post-P2-17):
   ...
   - Inbound ``event`` messages on an ``adapter:feishu/<app_id>`` topic
     ...
     `forward_to_new_chain/2` → `Esr.Session.Admin.Process.admin_peer/1`
  ...
  `:feishu_app_adapter_<app_id>` in `Esr.Session.Admin.Process`.
  ...
  def forward_to_new_chain("adapter:feishu/" <> app_id, envelope) do
    ...
    case Esr.Session.Admin.Process.admin_peer(sym) do
      {:ok, pid} ->
